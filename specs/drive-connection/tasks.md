...
- [x] T1 — `pnpm add @googleapis/drive` (queda en `^20.2.0`). **NO añadir
...
- [x] T2 — **Spike de 5 min antes de construir encima:** comprobar que
      ...
      → **Resultado: el named import resuelve OK en runtime** (Node v24 ESM).
      No hizo falta el fallback de default import.

## Error de dominio (aditivo, no rompe nada)

- [x] T3 — Modificar `src/errors/app-error.ts`: añadir `DriveConnectionError`
...
- [x] T4 — Modificar `src/errors/app-error.test.ts`: test de
...
- [x] T5 — Modificar `src/config/env.ts`: `interface DriveCredentials`, campo
...
- [x] T6 — Modificar `vitest.config.ts`: añadir al bloque `env` las tres
      ...
      Cubre: R16.
- [x] T7 — Modificar `src/config/env.test.ts`: fixture `baseEnv` (`DATABASE_URL`
...
- [x] T8 — Crear `src/lib/drive.ts`: `driveScope`, `createDriveAuth`,
      ...
      R19.
- [x] T9 — Crear `src/lib/drive.test.ts` (unitarios, dobles en el seam, sin
...
- [x] T10 — Crear `src/plugins/drive.ts`: `declare module` para tipar
...
- [x] T11 — Modificar `src/app.ts`: `app.register(drivePlugin)` en la zona de
      infraestructura, después de `prismaPlugin` y antes de los módulos.
      Cubre: R7.
- [x] T12 — Crear `src/plugins/drive.test.ts`: `buildApp()` + `await
...
- [x] T13 — Modificar `src/modules/health/health.routes.ts`: `GET /health/drive`
...
- [x] T14 — Modificar `src/modules/health/health.test.ts`: describe nuevo que
...
- [x] T15 — Modificar `src/architecture.test.ts`: (a) añadir `lib/drive.ts` y
...
- [x] T16 — Crear `scripts/get-drive-refresh-token.mjs`: `import 'dotenv/config'`,
...
- [x] T17 — Modificar `.env.example`: las tres variables con **placeholders**
      (nunca valores reales), con un comentario que apunte a `design.md` §10 para
      obtenerlas. Cubre: R14.
- [x] T18 — Modificar `docs/api-contract.md`: `GET /health/drive` en la tabla
...
- [x] T19 — Modificar `docs/architecture.md`: añadir **ADR-007** (borrador
...
- [x] T20 — Modificar `docs/stack.md`: las tres variables en la tabla
...
- [x] T21 — `pnpm test`: suite completa en verde, **sin credenciales reales de
      Drive y sin red**. Los 35 existentes siguen pasando. Cubre: R16.
- [x] T22 — `pnpm typecheck`, `pnpm lint` y `pnpm format:check` en verde
      (recuerda: el `.mjs` de T16 sí lo revisa Prettier). Cubre: R18.
- [x] T23 — `bash ./init.sh` → termina con `[OK] Entorno listo`. Cubre: R18.
- [x] T24 — Escribir el mapa de trazabilidad `R<n>` → test concreto en
