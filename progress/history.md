...
- 2026-08-16 — F16 `statement-balance`: el **saldo de la cuenta** entra en el sistema — lo escribes como segunda línea de preámbulo `saldo;1500,00` debajo de la del `iban;`, y el extracto parseado lo devuelve como `accountBalance`, campo del contrato común y **distinto del `balance` de cada movimiento** (que en este banco sigue siendo `null` para siempre). Si algún mes se te olvida, no falla nada; si la línea está y el número no se entiende, sale en `unparsedRows` en vez de perderse. La fila `Saldo` del final del archivo ya no se lee: una sola forma de escribirlo (ADR-019) → [resumen](summaries/statement-balance.md)

- 2026-08-18 — F18 `n26-statement`: **N26 es el tercer banco que entra en el sistema** — su `.csv` se lee tal y como lo exporta el banco (solo con tus dos líneas de preámbulo `iban;` y `Saldo;` arriba, con `;` aunque el fichero separe por comas) y sus movimientos entran por `POST /api/parser/n26`, con `POST /api/import` dejando de reportarlos como `skipped`. Trae el **primer lector de CSV de verdad** del repo —comas y saltos de línea dentro de un campo entrecomillado, `""` literal—, que vive **dentro** del módulo del banco y no en `lib/`, y un **concepto compuesto** (contraparte + referencia) porque N26 no exporta ninguna columna de concepto: ningún movimiento sale nunca con el concepto vacío (ADR-020) → [resumen](summaries/n26-statement.md)

- 2026-08-18 — F21 `iban-normalization`: el IBAN deja de guardarse **literal** — un solo normalizador+validador en [`src/lib/iban.ts`](../src/lib/iban.ts) al que llaman los **tres bancos** y las **dos** entradas del alta de cuenta, así que `'ES00 1111 …'` y `'es001111…'` son **la misma cuenta** y ya no se crean dos en silencio. Además el IBAN **se comprueba** (forma, longitud del país y dígito de control mod-97): uno mal tecleado rechaza el fichero entero con `INVALID_IBAN` (422) diciendo la línea y el problema por su nombre, sin crear ninguna cuenta. Sin migración (justificada: las cuentas de hoy ya están en forma canónica y la normalización es idempotente) y sin tocar los movimientos ya importados; `:` sigue fallando y `iban;<IBAN>` sigue siendo la única forma (ADR-021) → [resumen](summaries/iban-normalization.md)

- 2026-08-19 — F19 `openbank-statement`: **Openbank es el cuarto banco que entra en el sistema**, y el de más volumen — su archivo, que se llama `.xls` pero por dentro es **una página HTML**, se lee **tal y como lo descargas**, sin convertirlo a mano cada mes y **sin añadir ninguna librería**: entran sus **dos años de histórico enteros** por `POST /api/parser/openbank`, y `POST /api/import` deja de reportarlos como `skipped`. Es el primer banco que trae el **saldo de la cuenta en el propio archivo** (tú no escribes la línea `saldo;`); lo único que escribes es el IBAN, **una sola vez**, en un comentario HTML de la primera línea. La regla «siempre UTF-8» queda **acotada, no rota**: lo que escribes tú sigue yendo en UTF-8 y lo que emite el banco se lee con **su** codificación (cp1252), que el archivo tiene que **declarar** — si deja de declararla, se rechaza entero (`UNEXPECTED_ENCODING`) en vez de entrar con los acentos rotos en silencio (ADR-022). El saldo tras cada movimiento **existe en el archivo y no se guarda**, a propósito → [resumen](summaries/openbank-statement.md)

- 2026-08-19 — F22 `encoding-mismatch-guard`: cuando el archivo de Openbank llega **reguardado en otra codificación** —lo que le pasa a cualquiera que lo abra con Visual Studio Code y le dé a guardar—, el error **dice la verdad y dice qué hacer**: que el fichero se ha vuelto a guardar en UTF-8 declarando todavía `iso-8859-1`, y con qué codificación hay que guardarlo (**Western/Windows-1252**); y si además ya trae los caracteres destrozados (`U+FFFD`), que **están perdidos** y hay que volver a descargarlo del banco. Hasta hoy ese archivo se rechazaba diciendo «no es un extracto de este banco», que era **falso** y costó una vuelta entera de diagnóstico. Se detecta por un **hecho** —un cp1252 con acentos no es UTF-8 válido—, nunca adivinando la codificación ni reparando el fichero; un archivo legítimo entra exactamente igual (200 movimientos, 0 sin parsear) y los otros tres bancos no cambian (ADR-023) → [resumen](summaries/encoding-mismatch-guard.md)

- 2026-08-19 — F23 `no-real-data-blind-spot`: el guardián que impide que tus datos acaben en el repositorio **mira ya todos tus ficheros de banco**, no solo los de ciertas extensiones — el `.xls` de Openbank (el único que trae **nombres de personas**, y el que nunca se había abierto: por ahí pasó la fuga de importes de la F19 con la suite en verde) **entra y se compara** como cualquier `.csv`. Lo que se captura lo deciden ahora **los bytes** —¿se leen como texto?— y no una lista de extensiones que dentro de un año vuelve a dejar un hueco; de un fichero de marcado se compara **lo que dice, no sus etiquetas**, y los dos binarios de verdad (el `.xlsx` de Bankinter y el `.pdf` de Trade Republic) quedan fuera para no meter ruido. Y lo que no puede vigilar **te lo dice**: un banco cuya carpeta tiene ficheros y ninguno legible sale **por su nombre en la salida de `./init.sh`**, en toda ejecución, y si aparece uno nuevo la suite se pone **roja** — se acabó el verde silencioso sobre lo que no ha mirado. Sus mensajes dejan además de transcribir tu importe: dicen `archivo:línea` y el tipo, nunca el valor. **Trade Republic sigue sin vigilancia** (su extracto es un PDF ilegible sin las fuentes): queda **declarado y a la vista**, no resuelto (ADR-017 revisado) → [resumen](summaries/no-real-data-blind-spot.md)

- 2026-08-20 — F24 `guardian-own-words`: el guardián **deja de confundir sus propias frases con tus datos** — cada vez que un archivo tuyo se rechaza, el volcado de `var/parsed/` se queda con el **motivo del rechazo que escribe el propio programa** (la fórmula del cuadre, el formato de fecha que se espera), y como la documentación publica esas mismas frases, el guardián las leía como «una frase de tu extracto copiada en los docs»: **270 avisos falsos en una sola ejecución**, con el aviso bueno perdido entre los otros 269, y le pasaba a **cualquier banco**. Ahora son **cero**, y **sin bajar la guardia**: una frase solo se da por nuestra si además de estar fuera de comillas está **literal en nuestro código de producción** (las dos condiciones, nunca una), los cinco importes que ese mensaje lleva dentro se siguen vigilando igual —la capa de importes no se ha tocado— y si mañana copias un importe o un concepto tuyo a un archivo del repositorio, se sigue cazando. Sin una sola excepción nueva: ni un archivo ignorado, ni una frase quitada de los docs. Y **Trade Republic deja de estar sin vigilancia** —lo que la línea de la F23 dejó declarado y a la vista—: tu `.json` escrito a mano sí se lee, así que el banco sale de la lista de huecos, que queda **vacía**. Un silencio que encontró la review por el camino (un concepto con **apóstrofo dentro** dejaba de compararse) quedó cerrado de raíz y con test (ADR-017 revisado) → [resumen](summaries/guardian-own-words.md)

- 2026-08-20 — F20 `trade-republic-product-file`: **Trade Republic es el quinto banco que entra en el sistema**, y el único que entra **sin parser de lo que emite el banco** — su extracto es un PDF con dos apuntes al mes, así que en vez de leerlo **escribes tú un `.json` al mes** con la foto del saldo, lo dejas en su carpeta de Drive y entra por `POST /api/parser/trade-republic`, igual que los productos de MyInvestor y sin tocar la base de datos. Tienes un **archivo copiable** (`docs/plantillas/trade-republic-cuenta-remunerada.json`, **retirado del repositorio el 2026-08-22** por decisión del humano: sus plantillas viven en Drive; el formato sigue siendo el de [`docs/trade-republic-product-files.md`](../docs/trade-republic-product-files.md)) donde **todos** los valores son marcadores `<…>`, atado byte a byte al documento y al test para que las tres copias no puedan separarse, y lo que te dejes sin rellenar se rechaza **nombrando los diez campos**, no muriendo con un «JSON inválido». Lo que ningún banco tenía hasta hoy: **el archivo se comprueba a sí mismo** —`saldo inicial + entradas − salidas + intereses = saldo final`, en céntimos enteros y con 1 céntimo de tolerancia— y un descuadre **lo rechaza, no lo avisa**: en la prueba real el abono de intereses acabó escrito en la casilla de entradas, exactamente la errata que el documento avisaba, y el cuadre lo paró en vez de dejarlo entrar en silencio. Esto es **provisional y está escrito que lo es**: el día que la cuenta tenga movimientos de verdad se escribe el parser del PDF y el `.json` a mano desaparece (ADR-024). Y es **la primera feature que pasa por el checkpoint de la prueba real** (C4 bis), que en una tarde encontró dos cosas que la suite en verde no veía: una documentación que afirmaba como hecho algo **falso** sobre su extracto (el resumen no da los datos del mes, da los del **periodo**: seguirla daba descuadre garantizado todos los meses) y un guardián que soltaba **270 avisos falsos** —de ahí salió la F24—. Pasada final con tu archivo ya corregido: **tu cuenta entra entera, el cuadre no protesta y el `.pdf` se lista como ignorado con su motivo, no como fallo**. Queda **Revolut** para cerrar el inventario: su fichero ya se baja de Drive, pero nadie lo parsea todavía (E4: 5 de 6) → [resumen](summaries/trade-republic-product-file.md)

- 2026-08-20 — F25 `reimport-from-local-copy`: **`procesados/` deja de ser una puerta de un solo sentido** — hasta hoy, en cuanto un archivo entraba ahí desaparecía del sistema para siempre y la única salida era **entrar en Drive y moverlo a mano**, que es exactamente lo que tuviste que hacer con Bankinter para rescatar sus 39 movimientos. Ahora se reimporta **desde la copia local** que cada descarga deja en `var/drive-read/<banco>/<año>/` (`POST /api/import/local`, con `{ bank, year, name }` opcionales), y esa vía **no toca tu Drive**: no descarga, no lista, no mueve y no borra —ni una llamada, comprobado con espías—, así que puedes repetirla sin miedo y la segunda pasada te sale como `duplicates`, no como movimientos nuevos. Y se tapa el silencio que dejó vivo el diagnóstico: **un archivo del que no entra ni un movimiento deja de contarse como importado y deja de moverse** — si se lee sin errores y no trae ni una línea, o si trae líneas y **ninguna** se puede interpretar (que es lo que parece un formato del banco que ha cambiado), sale como fallo con su motivo y **se queda pendiente**; en corto: a `procesados/` solo se llega **si al menos una de sus filas está en la base de datos**, ahora o de antes. La importación de cada mes no cambia ni una línea. **Queda un límite vivo y escrito** en el contrato y en el ADR-025: repetir la llamada **sin cuerpo** solo es inocuo **mientras el parser de ese banco sea el mismo** con el que se importó el archivo — si ha cambiado (mejoró y hoy lee una fila que antes no leía), la renumeración del orden dentro del día **puede insertar copias**, así que sobre copias antiguas se pide **el archivo concreto** y se mira el recuento (ADR-025) → [resumen](summaries/reimport-from-local-copy.md)

- 2026-08-20 — F26 `savings-account-as-product`: **tu cuenta remunerada ya vive en la base de datos**, no en un volcado — el `.json` que escribes cada mes deja de morir en `var/parsed/` y entra por el **botón de siempre** (`POST /api/import`, que ahora distingue un archivo de producto de un extracto), donde queda guardada como un producto más con **una foto por mes**: saldo inicial, entradas, salidas, intereses y saldo final, los cinco **tal y como los tecleas** — no se calcula ni se redondea un céntimo, ni siquiera el saldo final, y el cuadre que ya te protegía de las erratas sigue **rechazando antes de escribir nada**: un mes que no cuadra no deja rastro (ni cuenta, ni foto, ni movimiento a `procesados/`) y te dice el motivo entero para que lo corrijas y lo vuelvas a subir. **Subir el mismo mes dos veces no te duplica nada y el mes siguiente añade una fila**, sin crearte una segunda cuenta: la identidad son **el nombre y la fecha que escribes tú**, y no algo que el sistema renumere por su cuenta — que es exactamente la trampa en la que cae el orden de los movimientos dentro del día y por la que la F25 dejó un aviso. **Tu plantilla no cambia** (ni un campo nuevo, IBAN incluido) y `var/parsed/` sigue ahí con otro oficio: pasa a ser el **ensayo**, para mirar qué ha entendido el sistema de un archivo **sin** que se escriba nada. La **prueba real** lo confirmó con tu archivo: entró creando cuenta y mes, se movió a `procesados/` **después** de guardarse de verdad, el cuadre de los cinco importes se sostiene consultado **dentro de la base**, y la reimportación de la misma copia salió `created: false` en cuenta y en foto, dejando **1 producto y 1 foto**, con tus 4 cuentas y tus 455 movimientos intactos. Sigue sin haber **ninguna pantalla ni consulta** que te lo enseñe —escribirlo era esta feature, leerlo es la siguiente— y los 5 `.json` de **MyInvestor siguen fuera**, en su feature hermana, que reutilizará esta misma vía (ADR-026) → [resumen](summaries/savings-account-as-product.md)

- 2026-08-20 — F27 `tests-dont-touch-real-db`: **la suite deja de escribir en tu base de datos** — hasta hoy `pnpm test` corría contra la tuya, y el día que se miró de verdad había dentro **15 cuentas, 5 movimientos y 140 productos de inversión** sintéticos dejados por las pasadas de una sola tarde (se limpiaron a mano, con copia previa y comprobando fila a fila que ninguna era tuya). Ahora **cada worker corre contra una base desechable** (`gastos_test_<n>`, en el mismo contenedor, clonada de una plantilla migrada), y **tú no tienes que arrancar nada nuevo**: se crean solas, `./init.sh` no cambia ni una línea y `docker compose up -d` sigue siendo todo lo que hace falta. Los tests **siguen probando contra un PostgreSQL de verdad** —ninguno se ha convertido en simulación— y de tu base **no se borra nunca nada**: el único código que la abre hace `select`. Y esto **no era higiene**: esa misma concurrencia de varios tests sobre una base compartida hacía fallar `./init.sh` **4 de cada 13 veces** con un 500 en el listado de movimientos (una cuenta borrada por otro archivo justo mientras se listaba), un fallo que parecía aleatorio y no lo era; reproducido a voluntad, y muerto: **18 pasadas seguidas, 18 verdes**. Dos guardianes lo sostienen sin depender de que nadie se acuerde: uno pone **en rojo el archivo** que deja una sola fila detrás, y otro compara una **foto de solo lectura** de tu base antes y después de cada pasada —recuentos **y** contadores, así que caza hasta el test que inserta y borra— y tiñe la pasada entera si algo se movió. Lo que cuesta: la suite pasa de **~6,1 s a ~7,4 s** (ADR-027) → [resumen](summaries/tests-dont-touch-real-db.md)

- 2026-08-21 — F28 `half-erased-marker-message`: si al rellenar la plantilla te dejas **medio corchete** de un marcador —`"<2026-08-01"` en vez de `2026-08-01`—, el error te dice **eso mismo**: que te dejaste un símbolo suelto, **en qué campo**, y con el valor delante para que veas el carácter que sobra. Antes te decía «fecha inválida, se espera el formato AAAA-MM-DD», que era verdad y te mandaba a mirar el formato, que estaba bien: por eso ese mismo `<` **sobrevivió a una corrección entera** el 2026-08-20 y costó una segunda vuelta por un carácter. Vale para **los once campos**, no solo las fechas: un importe con el corchete pegado (`"<4006.40"`, `"0>"`) también se dice por su nombre en vez de «se espera un número». El símbolo **en medio** del texto NO cuenta —solo al principio o al final, tras recortar espacios—, así que un `name` tuyo puede llevarlo sin que nadie te moleste; el mensaje del marcador **entero** (`<…>`) no cambia ni una letra; el archivo se **rechaza igual** (no te lo arregla ni adivina el valor), solo cambia el motivo; y lo que ya estaba mal de verdad —una fecha que no existe, un número con letras— sigue diciendo exactamente lo de siempre, con test de no-regresión. **No se comparte con MyInvestor a propósito**: la norma del proyecto es **un parser por banco** y el marcador es forma del fichero, no de la plantilla, así que se queda en Trade Republic hasta que alguien lo necesite dos veces → [resumen](summaries/half-erased-marker-message.md)

- 2026-08-21 — F29 `myinvestor-products-to-db`: **tus 5 productos de MyInvestor ya están en la base de datos** — subes los mismos `.json` de siempre por `POST /api/import` y dejan de salir «ignorados»: tu **fondo, tu ETF y tu cartera gestionada** entran con **una valoración por fecha**, y tus **dos depósitos no guardan ni una valoración**, porque no fluctúan: sus condiciones (principal, tipo de interés, ganancia esperada y vencimiento) viven en el **propio producto** (ADR-012), y por eso su respuesta trae `snapshot: null` en vez de fingir una foto que no existe. **No escribes ni un campo nuevo** y **el parser no se ha tocado**: esta feature añadió **quién escribe**, no cómo se lee, y sin migración —las tablas estaban ahí desde la F9, esperando escritor desde hacía veinte features—. **Subir el mismo mes dos veces no duplica nada y el mes siguiente añade una valoración** sin crearte un producto nuevo: la identidad son **el nombre y la fecha que escribes tú**, nunca algo que el sistema renumere (el aviso de la F25). Y **o entra entero o no entra**: un archivo con una errata no deja medio producto detrás ni se mueve a `procesados/`. La **prueba real** lo confirmó con tus cinco archivos: los cinco `imported` y creados, la base enseñando 1+1+1 valoraciones para lo que fluctúa y **0 para los dos depósitos**, y la reimportación entera saliendo `created: false` sin duplicar nada —con tu **cuenta remunerada de la F26 intacta** (su foto, sin valoraciones) y tus **4 cuentas y 455 movimientos** sin tocar → [resumen](summaries/myinvestor-products-to-db.md) · [prueba real](explorations/prueba-real-myinvestor-productos-2026-08-21.md)

- 2026-08-23 — F30 `myinvestor-template-marker-guard`: **un `.json` de producto de MyInvestor con un campo sin sustituir deja de entrar en silencio** — si te dejas el marcador de la plantilla (`<nombre del producto, tal y como lo llamas siempre>`), el archivo se **rechaza y el motivo te dice qué campo es**, en vez de guardarte un producto llamado literalmente así. **Los expuestos eran dos y está medido, no supuesto**: de las 13 claves de tus dos plantillas, once ya se rechazaban solas —el `type` por no ser uno de los cuatro, las fechas por no ser `AAAA-MM-DD` y los ocho importes por venir como texto— y las que entraban en verde eran **el nombre y la divisa**, los dos de texto libre (la auditoría solo señalaba el nombre; `currency` apareció al medir, y el revisor reprodujo los 99 casos con el parser de antes y el de ahora). **El nombre es el que importa**: desde la F29 es la **identidad** del producto, así que uno que entra a medio rellenar se queda en la base y el mes siguiente, ya con el nombre bueno, **te crea otro producto en vez de continuar el mismo** —la serie partida en dos y la primera foto colgando de un nombre que no existe—. Y el riesgo no era teórico: **el 2026-08-15 subiste un archivo con los valores del ejemplo y sobrevivió a una revisión humana**, porque eran valores legales, solo que falsos. También caza el **marcador a medio borrar** (`"<nombre del producto"`), la errata número uno al rellenar a mano, con la misma regla de la F28: cuenta al principio o al final, **en mitad del texto no**, así que un producto llamado `Cartera 3 > 2` entra sin que nadie te moleste. **Tus 5 productos siguen entrando igual** (comprobado con tus propios ficheros: 5 de 5, los cuatro tipos), no escribes ni un campo nuevo, el parser **no adivina ni repara**, lo ya guardado no se toca y **ningún otro banco cambia**. La comprobación **no se comparte con Trade Republic** y la decisión se volvió a tomar, no se heredó: cada banco tiene la suya porque **la política diverge** —TR la pasa por todos sus campos, MyInvestor solo por los dos expuestos—, y queda escrito que el tercer banco con plantilla a mano **copia el patrón y mide antes** → [resumen](summaries/myinvestor-template-marker-guard.md)

- 2026-08-26 — F31 `real-account-balance`: **cada cuenta corriente sabe ya cuánto dinero tiene dentro**, no solo cuánto ha variado desde cero — el saldo del extracto, que los parsers leían y el importador tiraba, se guarda **una sola vez** como ancla (saldo + fecha, tal cual viene, no despejado: así el orden en que entren los archivos no lo puede envenenar) y a partir de ahí el saldo **se mantiene solo** con los movimientos que van entrando, sin que escribas nada nunca más. Un segundo extracto de una cuenta ya anclada **no reescribe** el ancla. La **precedencia sigue intacta**: en las cuentas cuyo archivo trae saldo manda el archivo —más lo que haya llegado después—, y lo que cambia es que el cálculo deja de ser un fallback que nadie miraba y pasa a hacerse **siempre, en todas las cuentas**, que es lo que sostiene a N26 y MyInvestor hoy y lo que hará de testigo cuando llegue la F32. Se implementó en **cuatro lotes** y con una migración que **solo añade columnas** → [resumen](summaries/real-account-balance.md)

- 2026-08-26 — F34 `guardian-knows-our-own-filenames`: **parsear tus archivos de verdad deja de ponerte la suite en rojo** — cada parseo real de Trade Republic dejaba dentro del volcado el **nombre del archivo** (`cuenta-remunerada-2026-07-01.json`), que es la convención que **el propio proyecto te dice que uses**, y el guardián de datos reales lo leía como una frase de tu extracto: de ahí salían los **52 avisos** de esta semana, todos del mismo trigrama y todos señalando textos **nuestros**. Ahora sabe distinguirlos, y **sin aflojar un milímetro**: un texto solo pasa si cumple **las dos** condiciones a la vez —estar bajo un campo que sabemos que guarda un nombre de archivo (`file`) **y** encajar **entero** con un patrón publicado en nuestra propia documentación—, y solo valen los patrones cuyo **único hueco es una fecha**, así que lo que se exime es, por construcción, texto nuestro más dígitos. **No hay lista de excepciones a mano**: las convenciones se **leen de los `docs/`** (un banco nuevo entra por documentarse, no por tocar el guardián) y cuando deja pasar algo se puede preguntar **qué** dejó pasar y **qué página lo autoriza**. Que sigue cazando está probado, no afirmado: un test mete un dato tuyo dentro de un nombre de archivo y exige aviso con **archivo y línea** y **sin escribir el valor**; el reviewer lo puso a prueba con **tres mutaciones** —aflojar cada mitad de la regla y apagar la exención entera— y la suite se puso roja las tres veces, con los 52 avisos volviendo **exactos, ni uno más**. Comprobado banco por banco y no supuesto: de tus siete volcados solo dos guardan nombres, Trade Republic exime 24 de 26 y **MyInvestor exime 0 de 5** (su convención lleva **tu** nombre de producto dentro, así que se sigue vigilando entera). **No renombras nada** ni cambias cómo escribes tus archivos, y no se tocó ni la aplicación, ni `var/`, ni la F31, ni la F33: **950 tests en verde** con tu volcado real delante (ADR-017 revisado) → [resumen](summaries/guardian-knows-our-own-filenames.md)

- 2026-08-29 — F33 `tests-dont-touch-real-var`: **pasar los tests deja de tocar tus datos** — un test construía la aplicación **real** y llamaba de verdad a la ruta de Trade Republic sin inyectarle directorios de prueba, así que cada pasada de la suite **parseaba tus archivos descargados del banco y te reescribía el volcado** `var/parsed/trade-republic/2026/products.json` sin que te enteraras (y de rebote envenenaba al guardián de datos reales, que a partir de la segunda ejecución dejaba el repositorio en rojo sin que nadie hubiera escrito nada mal). Ese test comprueba ahora lo mismo —que la ruta está registrada en la app de verdad— **sin ejecutar nada**. Y para que no vuelva a colarse por descuido, la suite **se vigila a sí misma**: fotografía `var/` antes de empezar y al terminar, y si un solo archivo cambió de contenido, de tamaño o **solo de fecha**, la pasada acaba en **rojo** diciendo **qué archivo** se movió y nunca lo que dice dentro. De propina se arregló un fallo heredado y serio: el **guardián de tu base de datos de la F27** llevaba desde el día que se escribió **detectando pero sin poder tumbar una pasada** —avisaba e `./init.sh` seguía diciendo que todo pasaba—; los dos guardianes comparten ahora el mismo mecanismo, que sí pone la pasada en rojo → [resumen](summaries/tests-dont-touch-real-var.md)

- 2026-08-30 — F35 `client-errors-are-not-500`: **una petición mal mandada deja de parecer una avería del servidor** — cuerpo vacío con cabecera de JSON, JSON mal escrito, tipo de contenido no soportado o cuerpo demasiado grande salían todos como `500 INTERNAL_SERVER_ERROR`, indistinguibles de un backend caído: ahora cada uno responde con **su propio código de estado** (400, 415, 413), con un `code` **derivado del estado HTTP** en vez de una lista escrita a mano que haya que ampliar, y con el **mensaje propagado solo si es la plantilla estática que declara Fastify**, para no filtrar detalle interno. Los errores nuestros y los de validación de esquema siguen exactamente igual, y un fallo inesperado de verdad sigue siendo un 500 genérico con el detalle solo en el log → [resumen](summaries/client-errors-are-not-500.md)
