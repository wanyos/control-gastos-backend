...
export interface N26RoutesOptions {
  /**
...
/**
 * HTTP layer for the N26 parser (parse + model only; no DB, no Drive move):
 *   POST /api/parser/n26 -> parse the local statement copies and dump JSON
 *
 ...
 */
export default async function n26Routes(fastify: FastifyInstance, options: N26RoutesOptions = {}) {
  ...
  fastify.post('/n26', async () => {
    return parseLocalN26Copies(sourceBaseDir, dumpBaseDir)
  })
}
