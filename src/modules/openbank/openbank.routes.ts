...
export interface OpenbankRoutesOptions {
  /**
...
/**
 * HTTP layer for the Openbank parser (parse + model only; no DB, no Drive move):
 *   POST /api/parser/openbank -> parse the local statement copies and dump JSON
 *
 ...
 */
export default async function openbankRoutes(
  fastify: FastifyInstance,
  options: OpenbankRoutesOptions = {},
) {
  ...
  fastify.post('/openbank', async () => {
    return parseLocalOpenbankCopies(sourceBaseDir, dumpBaseDir)
  })
}
