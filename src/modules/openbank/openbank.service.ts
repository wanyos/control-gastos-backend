...
import type {
  FailedFile,
  IgnoredFile,
  OpenbankParseRunResult,
  ParsedStatementSummary,
} from './openbank.types.js'

const bankName = 'openbank'

/**
 * The single entry of this bank: the statement of its current account. The
 * extension is `.xls` and the content is NOT an Excel — it is an HTML page — so
 * the extension says which parser applies and nothing more; what the file is
 * inside is the parser's business.
 */
const statementExtension = '.xls'

/**
 * Parses every local Openbank copy (the ones the drive-read feature dropped under
 * `<sourceBaseDir>/openbank/<year>/`) and, for each statement, writes the structured
 * result to a JSON dump under `<dumpBaseDir>/openbank/<year>/`.
 *
 ...
 */
export async function parseLocalOpenbankCopies(
  sourceBaseDir: string,
  dumpBaseDir: string,
): Promise<OpenbankParseRunResult> {
...
async function parseAndDump(
  ...
  const result = parseOpenbankStatement(content)
