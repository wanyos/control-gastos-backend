...
describe('parseLocalMyinvestorCopies', () => {
  ...
  it('reports the extensions it does not handle in ignored, not as failures (R49, R50)', async () => {
    ...
    // The product .json files also land here until the feature that reads them
    // exists: nobody can interpret them yet, and that is not a failure.
    await writeLocalCopy(sourceDir, '2026', 'fondo.json', '{}')
    ...
    expect(result.ignored.map((entry) => entry.file)).toEqual([
      'deposito.txt',
      'fondo.json',
      'hoja.xlsx',
    ])
